...
#define BRICKPOOL_MEMSIZE	( 2048U * 1024U * 1024U )	// 256 Mo
...
,   _noiseAmplitude (0.1f)
,   _noiseFrequency (32)
,   _noisePersistancy (2.f)
,   _useRealNormals (false)
...
void SampleCore::init()
{
	...
	setNoiseAmplitude(_noiseAmplitude);
	setNoiseFrequency(_noiseFrequency);
	setNoisePersistancy(_noisePersistancy);
	setUseRealNormals(_useRealNormals);
 ...
 ******************************************************************************/
void SampleCore::setNoiseFrequency (int pValue)
{
	_noiseFrequency = static_cast<float>( pValue) ;
	int noiseSkip = 0;
	if (_noiseFrequency >4.f)
	{
		noiseSkip = static_cast<int>(log2(_noiseFrequency/4));
		_noiseFrequency = 4.f;
	}

    // Update DEVICE memory with "voxel scale"
    GV_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNoiseFrequency, &_noiseFrequency, sizeof( _noiseFrequency ), 0, cudaMemcpyHostToDevice ) );
	GV_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNoiseSkip, &noiseSkip, sizeof( noiseSkip ), 0, cudaMemcpyHostToDevice ) );
 ...
 * @param pValue the desired value
 ******************************************************************************/
void SampleCore::setNoiseAmplitude (float pValue)
{
	_noiseAmplitude = pValue ;

    // Update DEVICE memory with "voxel scale"
    GV_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNoiseAmplitude, &_noiseAmplitude, sizeof( _noiseAmplitude ), 0, cudaMemcpyHostToDevice ) );
...
/******************************************************************************
 * Sets the noise persistancy
 *
 * @param pValue the desired value
 ******************************************************************************/
void SampleCore::setNoisePersistancy (float pValue)
{
	_noisePersistancy = pValue ;

    // Update DEVICE memory with "voxel scale"
    GV_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNoisePersistancy, &_noisePersistancy, sizeof( _noisePersistancy ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setUseRealNormals (bool pValue)
{
	_useRealNormals = pValue ;

    // Update DEVICE memory with "voxel scale"
    GV_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cUseRealNormals, &_useRealNormals, sizeof( _useRealNormals ), 0, cudaMemcpyHostToDevice ) );
