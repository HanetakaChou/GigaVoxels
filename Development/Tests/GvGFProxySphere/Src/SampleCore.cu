...
#define BRICKPOOL_MEMSIZE	( 1536U * 1024U * 1024U )	// 256 Mo
...
******************************************************************************/
void SampleCore::setAncestorLevel(int level)
{
	_ancestorLevel = level;

    // Update DEVICE memory with "voxel scale"
    GV_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNumberOfAncestors, &_ancestorLevel, sizeof( _ancestorLevel ), 0, cudaMemcpyHostToDevice ) );
