...
void Qtfe::onSaveAsButtonClicked()
{
	...
	QString defaultDirectory = GvUtils::GsEnvironment::getDataDir( GvUtils::GsEnvironment::eTransferFunctionsDir ).c_str();
	QString name = QFileDialog::getSaveFileName( 0, QString(), defaultDirectory, "*.xml" );
	if ( name.isEmpty() )
	{
		return;
	}
...
void Qtfe::onLoadButtonClicked()
{
	...
	QString defaultDirectory = GvUtils::GsEnvironment::getDataDir( GvUtils::GsEnvironment::eTransferFunctionsDir ).c_str();
	QString name = QFileDialog::getOpenFileName( 0, QString(), defaultDirectory, "*.xml" );
	if ( name.isEmpty() )
	{
		return;
	}
