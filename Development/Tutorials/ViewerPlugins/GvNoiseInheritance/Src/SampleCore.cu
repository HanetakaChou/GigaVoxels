...
void SampleCore::init()
{
	...
	setAmplitudeCoefficient( 0.85f );
	setFrequencyCoefficient( 5.f );
...
void SampleCore::setOuterRadius( float pValue )
{
	...
	// Update device memory
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cR, &_outerRadius, sizeof( _outerRadius ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setInnerRadius( float pValue )
{
	...
	// Update device memory
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cr, &_innerRadius, sizeof( _innerRadius ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setAmplitudeCoefficient( float pValue )
{
	...
	// Update device memory
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cAmplitudeCoefficient, &_amplitudeCoefficient, sizeof( _amplitudeCoefficient ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setFrequencyCoefficient( float pValue )
{
	...
	// Update device memory
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cFrequencyCoefficient, &_frequencyCoefficient, sizeof( _frequencyCoefficient ), 0, cudaMemcpyHostToDevice ) );
