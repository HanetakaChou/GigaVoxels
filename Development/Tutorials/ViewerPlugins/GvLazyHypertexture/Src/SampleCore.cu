...
void SampleCore::init()
{
	...
	dataRepository += QDir::separator();
	dataRepository += QString( "GvLazyHypertexture" );
	dataRepository += QDir::separator();
...
void SampleCore::setNoiseFrequency (int pValue)
{
    ...
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNoiseFrequency, &_noiseFrequency, sizeof( _noiseFrequency ), 0, cudaMemcpyHostToDevice ) );
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNoiseSkip, &noiseSkip, sizeof( noiseSkip ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setNoiseAmplitude (float pValue)
{
    ...
    // Update DEVICE memory with "voxel scale"
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNoiseAmplitude, &_noiseAmplitude, sizeof( _noiseAmplitude ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setNoisePersistancy (float pValue)
{
    ...
    // Update DEVICE memory with "voxel scale"
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNoisePersistancy, &_noisePersistancy, sizeof( _noisePersistancy ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setUseRealNormals (bool pValue)
{
    ...
    // Update DEVICE memory with "voxel scale"
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cUseRealNormals, &_useRealNormals, sizeof( _useRealNormals ), 0, cudaMemcpyHostToDevice ) );
