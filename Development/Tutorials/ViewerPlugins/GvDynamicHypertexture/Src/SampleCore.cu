...
,	_noiseShellWidth( 0.f )
,	_noiseShellWidthUpperBound( 0.f )
,	_noiseFunctionModifier( 0 )
,	_noiseFillTheShape( 0 )
,	_noiseDisplacement( 0 )
,	_animate( false )
{
...
void SampleCore::init()
{
	...
	setVoxelSizeMultiplier( 1.f );
	// Noise parameter(s)
	setNoiseFirstFrequency( 9.f );
	setNoiseShellWidth( 1.f );
	setNoiseShellWidthUpperBound( 0.125f );
	setNoiseFunctionModifier( 0 );
	setDisplacement( 0 );
	setFillTheShape( 1 );
	setAnimation( true );
	// Nice effect of Normal displacement (with Perlin) and parameters :
	// - LOD 1 or 2
	// - TF_DynamicHypertexture_04.xml
	// - 11 (first frequency)
	// - 0.124 (noise max bound)
	// - 0.124 (noise upper bound)
}
...
/******************************************************************************
 * Get the noise Shell Width
 *
 * @return the noise Shell Width
 ******************************************************************************/
float SampleCore::getNoiseShellWidth() const
{
	return _noiseShellWidth;
}

/******************************************************************************
 * Set the noise Shell Width
 *
 * @param pValue the noise Shell Width
 ******************************************************************************/
void SampleCore::setNoiseShellWidth( float pValue )
{
	_noiseShellWidth = pValue;

	// Update device memory
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNoiseShellWidth, &_noiseShellWidth, sizeof( _noiseShellWidth ), 0, cudaMemcpyHostToDevice ) );
}

/******************************************************************************
 * Set the noise Shell Width upper bound
 *
 * @param pValue the noise Shell Width upper bound
 ******************************************************************************/
void SampleCore::setNoiseShellWidthUpperBound( float pValue )
{
	_noiseShellWidthUpperBound = pValue;

	// Update device memory
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNoiseShellWidthUpperBound, &_noiseShellWidthUpperBound, sizeof( _noiseShellWidthUpperBound ), 0, cudaMemcpyHostToDevice ) );
...
/******************************************************************************
 * Get the noise Shell Width upper bound
 ******************************************************************************/
float SampleCore::getNoiseShellWidthUpperBound ()
{
	return _noiseShellWidthUpperBound;
}

/******************************************************************************
 * Set the noise function modifier
 *
 * @param pValue the noise function modifier
 ******************************************************************************/
void SampleCore::setNoiseFunctionModifier( int pValue )
{
	_noiseFunctionModifier = pValue;

	// Update device memory
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNoiseFunctionModifier, &_noiseFunctionModifier, sizeof( _noiseFunctionModifier ), 0, cudaMemcpyHostToDevice ) );
}

/******************************************************************************
 * Get the noise function modifier
 ******************************************************************************/
int SampleCore::getNoiseFunctionModifier()
{
	return _noiseFunctionModifier;
}

/******************************************************************************
 * Set the displacement type
 *
 * @param pValue the displacement type
 ******************************************************************************/
void SampleCore::setDisplacement( int pValue )
{
	_noiseDisplacement = pValue;

	// Update device memory
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNoiseDisplacement, &_noiseDisplacement, sizeof( _noiseDisplacement ), 0, cudaMemcpyHostToDevice ) );
}

/******************************************************************************
 * Get the displacement type
 ******************************************************************************/
int SampleCore::getDisplacement()
{
	return _noiseDisplacement;
}

/******************************************************************************
 * Set the fill the shape boolean
 *
 * @param checked the fill the shape value
 ******************************************************************************/
void SampleCore::setFillTheShape( int pValue )
{
	_noiseFillTheShape = pValue;

	// Update device memory
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNoiseFillTheShape, &_noiseFillTheShape, sizeof( _noiseFillTheShape ), 0, cudaMemcpyHostToDevice ) );
...
/******************************************************************************
 * Get the fill the shape boolean
 ******************************************************************************/
int SampleCore::getFillTheShape()
{
	return _noiseFillTheShape;
}
...
bool SampleCore::initializeTransferFunction()
{
	...
	// Default filename
	//QString dataRepository = QCoreApplication::applicationDirPath() + QDir::separator() + QString( "Data" );
	//QString filename = dataRepository + QDir::separator() + QString( "TransferFunctions" ) + QDir::separator() + QString( "TF_DynamicHypertexture_02.xml" );
	QString filename = GsEnvironment::getDataDir( GsEnvironment::eTransferFunctionsDir ).c_str();
	filename += QDir::separator();
	filename += QString( "TF_DynamicHypertexture_02.xml" );
	QFileInfo fileInfo( filename );
	if ( ( ! fileInfo.isFile() ) || ( ! fileInfo.isReadable() ) )
	{
		// Idea
...
/******************************************************************************
 * Get the flag telling wheter or not animation is activated
 *
 * @return the flag telling wheter or not animation is activated
******************************************************************************/
bool SampleCore::hasAnimation() const
{
	return _animate;
}

/******************************************************************************
 * Set the flag telling wheter or not animation is activated
 *
 * @param pFlag the flag telling wheter or not animation is activated
******************************************************************************/
void SampleCore::setAnimation( bool pFlag )
{
	_animate = pFlag;

	// Update device memory
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cAnimation, &_animate, sizeof( _animate ), 0, cudaMemcpyHostToDevice ) );
}
