...
,	_maxVolTreeDepth( 0 )
{
	...
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNbCameraReflections, &cameraInBrick, sizeof( cameraInBrick ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setNumberOfReflections( int pValue ) 
{
    ...
    // Update DEVICE memory with "voxel scale"
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNbMirrorReflections, &_numberOfReflections, sizeof( _numberOfReflections ), 0, cudaMemcpyHostToDevice ) );
