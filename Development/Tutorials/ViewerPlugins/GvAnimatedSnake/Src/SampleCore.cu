...
void SampleCore::init()
{
	...
	//
	// Iterate thruogh array of directions
	for ( int k = 0; k < 20; k++ )
	{
		_directions[ k ] = make_uint3( 0 );
	}
	_oldDirection = 0;
	...
	dataRepository += QDir::separator();
	dataRepository += QString( "GvAnimatedSnake" );
	dataRepository += QDir::separator();
...
{
	// BEGIN : Animation pre-pass

	// Direction can take 8 values
	int dir = rand() % 8;
	if ( dir > 5 )
	{
		dir = _oldDirection;
	...
	// Iterate through array of directions
	//
	// Create "time shifting" animation by cycling direction
	for ( int k = 20; k > 0; k-- )
	{
		_directions[ k ].x = _directions[ k - 1 ].x;
		_directions[ k ].y = _directions[ k - 1 ].y;
		_directions[ k ].z = _directions[ k - 1 ].z;
	}
	
	uint3 newDir = _directions[ 0 ];
	...
	_directions[ 0 ] = newDir;

	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cDirections, &_directions, 20 * sizeof( _directions[ 0 ] ), 0, cudaMemcpyHostToDevice ) );

	// END : Animation pre-pass
...
bool SampleCore::initializeTransferFunction()
{
	...
	_transferFunction->bindToTextureReference( &colorCyclingLookupTable, "colorCyclingLookupTable", true, cudaFilterModeLinear, cudaAddressModeClamp );
...
void SampleCore::updateTransferFunction( float* pData, unsigned int pSize )
{
	...
	if ( _transferFunction != NULL )
	{
		...
		// Node need to update the cache because transfer function is applied during Rendering stage
		// and not during Data Production stage.
		//_pipeline->clear();
	}
}
