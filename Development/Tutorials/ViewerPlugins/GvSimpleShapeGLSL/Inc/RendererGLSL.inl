...
:	GvRendering::GsRenderer< TDataStructureType, TVolumeTreeCacheType >( pVolumeTree, pVolumeTreeCache )
...
::doRender( const float4x4& pModelMatrix, const float4x4& pViewMatrix, const float4x4& pProjectionMatrix, const int4& pViewport )
{
	...
	// Unmap GigaVoxels graphics resources from CUDA environment in order to be used by OpenGL
	GS_CUDA_SAFE_CALL( cudaGraphicsUnmapResources( 7, &_graphicsResources[ 0 ], 0 ) );
	...
	GS_CUDA_SAFE_CALL( cudaGraphicsMapResources( 7, &_graphicsResources[ 0 ], 0 ) );
...
::initializeShaderProgram()
{
	...
	QString vertexShaderFilename = shaderRepository + QDir::separator() + QString( "GvSimpleShapeGLSL" ) + QDir::separator() + QString( "gigaspace_vert.glsl" );
	...
	QString fragmentShaderFilename = shaderRepository + QDir::separator() + QString( "GvSimpleShapeGLSL" ) + QDir::separator() + QString( "gigaspace_frag.glsl" );
