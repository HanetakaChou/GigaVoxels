...
SampleCore::~SampleCore()
{
	...
	GV_CHECK_GL_ERROR();
	if ( _fullscreenQuadVertexBuffer )
	{
		glDeleteBuffers( 1, &_fullscreenQuadVertexBuffer );
		GV_CHECK_GL_ERROR();
	}
	if ( _fullscreenQuadVAO )
	{
		glDeleteVertexArrays( 1, &_fullscreenQuadVAO );
		GV_CHECK_GL_ERROR();
	}
