...
void SampleCore::init()
{
	...
	for( unsigned int i = 0; i < _nParticles; ++i ) {
		...
		p._speed.x = 0.f;
		p._speed.y = 0.f;
		p._speed.z = -0.0001f - static_cast <float> (rand()) / ( static_cast <float> (RAND_MAX/(.009f)));

		p._position.x = -.5f + static_cast <float> (rand()) / ( static_cast <float> (RAND_MAX/(1.f)));
		p._position.y = -.5f + static_cast <float> (rand()) / ( static_cast <float> (RAND_MAX/(1.f)));
		p._position.z = -.5f + static_cast <float> (rand()) / ( static_cast <float> (RAND_MAX/(1.f)));
...
void SampleCore::draw()
{
	...
	// Manage particles
		// Collision
	std::vector< unsigned int > vPrecisions;
	std::vector< float3 > vPositions;
	std::vector< float3 > vExtents;
	std::vector< float4x4 > vBasis;
	std::vector< float > vResults;
	
	vPrecisions.resize( _nParticles, 2 );
	vPositions.reserve( _nParticles );
	vExtents.reserve( _nParticles );
	vBasis.reserve( _nParticles );
	...
	for( unsigned int i = 0; i < _nParticles; ++i ) {
		...
		if( p->_position.z < -.5f ) {
			p->_position.z = -p->_position.z;
		}
		vPositions.push_back( p->_position );
		vExtents.push_back( p->_extents );
		vBasis.push_back( basis );
	}

	GvCollision::collision_BBOX_VolTree< DataStructureTypeKernel > (
			_pipeline->editDataStructure()->volumeTreeKernel,
			vPrecisions, 
			vPositions,
			vExtents,
			vBasis, 
		...
		// Draw particles
	for( unsigned int i = 0; i < _nParticles; ++i ) {
		glColor4f( 1.f, 1.f - vResults[i], 1.f - vResults[i], 1.f );
		
		float3 p1 = _particles[i]._position - _particles[i]._extents;
		float3 p2 = _particles[i]._position + _particles[i]._extents;
