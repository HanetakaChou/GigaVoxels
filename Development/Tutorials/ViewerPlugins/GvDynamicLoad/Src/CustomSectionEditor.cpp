...
void CustomSectionEditor::populate( GvViewerCore::GvvBrowsable* pBrowsable )
{
	...
	if ( pipeline != NULL )
	{
		...
		// Producer parameters
		_useHostCacheCheckBox->setChecked( pipeline->isHostCacheActivated() );
	}
}
 ...
 ******************************************************************************/
void CustomSectionEditor::on__useHostCacheCheckBox_toggled( bool pChecked )
{
	GvvApplication& application = GvvApplication::get();
	GvvMainWindow* mainWindow = application.getMainWindow();
	Gvv3DWindow* window3D = mainWindow->get3DWindow();
	...
	sampleCore->setHostCacheActivated( pChecked );
}
