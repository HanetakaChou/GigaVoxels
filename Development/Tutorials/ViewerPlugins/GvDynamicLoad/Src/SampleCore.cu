...
void SampleCore::init()
{
	...
	statusOK = _producer->initializeDataLoader(
		get3DModelFilename().c_str(),
	...
	// - data structure traversal parameters
	//_maxVolTreeDepth = 6;
	unsigned int dataResolution = _producer->getDataLoader()->getDataResolution().x;	// TODO : change that (uniform data)
 ...
 ******************************************************************************/
bool SampleCore::isHostCacheActivated() const
{
	return _producer->getDataLoader()->isHostCacheActivated();
}
 ...
 ******************************************************************************/
void SampleCore::setHostCacheActivated( bool pFlag )
{
	if ( isHostCacheActivated() != pFlag )
	{
		cudaError_t error = cudaDeviceSynchronize();
		if ( error == cudaSuccess )
		{	
			// Finalize data loader
			bool statusOK = _producer->finalizeDataLoader();
