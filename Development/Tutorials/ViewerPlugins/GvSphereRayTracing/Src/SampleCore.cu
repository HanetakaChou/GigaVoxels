...
void SampleCore::draw()
{
    ...
    const uchar4& color = /*_pipeline->getRenderer()*/_renderer->getClearColor();
...
void SampleCore::setNbSpheres( unsigned int pValue )
{
    ...
    // Update producer
    /*_pipeline->editProducer()*/_producer->setNbSpheres( pValue );

    // Update DEVICE memory
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNbSpheres, &_nbSpheres, sizeof( _nbSpheres ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setNbSpheresTotal( unsigned int pValue )
{
    ...
    // Update producer
    /*_pipeline->editProducer()*/_producer->setNbSpheres( nbSphereByBrick );
    ...
    // Update DEVICE memory
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNbSpheres, &nbSphereByBrick, sizeof( nbSphereByBrick ), 0, cudaMemcpyHostToDevice ) );
...
{
    /*_pipeline->editProducer()*/_producer->generateNewParticleBuffer();
...
void SampleCore::setUserDefinedMinLevelOfResolutionToHandleMode( bool pFlag )
{
	...
	// Update DEVICE memory
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cMinLevelOfResolutionToHandle, &_userDefinedMinLevelOfResolutionToHandle, sizeof( _userDefinedMinLevelOfResolutionToHandle ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setAutomaticMinLevelOfResolutionToHandleMode( bool pFlag )
{
	...
	if ( pFlag )
	{
		...
		// Update DEVICE memory
		GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cMinLevelOfResolutionToHandle, &minLevelOfResolutionToHandle, sizeof( minLevelOfResolutionToHandle ), 0, cudaMemcpyHostToDevice ) );
...
//
//	// Update DEVICE memory
//	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cAutomaticMinLevelOfResolutionToHandle, &_automaticMinLevelOfResolutionToHandle, sizeof( _automaticMinLevelOfResolutionToHandle ), 0, cudaMemcpyHostToDevice ) );
//
//	// Clear the cache
//	clearCache();
//}
...
void SampleCore::setSphereBrickIntersectionType( unsigned int pValue )
{
	...
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cSphereBrickIntersectionType, &_sphereBrickIntersectionType, sizeof( _sphereBrickIntersectionType ), 0, cudaMemcpyHostToDevice ) );
...
{
    return _sphereRadiusFader;///*_pipeline->getProducer()*/_producer->getSphereRadiusFader();
}
...
{
	///*_pipeline->editProducer()*/_producer->setSphereRadiusFader( pValue );
    _sphereRadiusFader = pValue;

    // Update DEVICE memory with "voxel scale"
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cSphereRadiusFader, &_sphereRadiusFader, sizeof( _sphereRadiusFader ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setGeometricCriteria( bool pFlag )
{
	...
	// Update DEVICE memory with "voxel scale"
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cGeometricCriteria, &_geometricCriteria, sizeof( _geometricCriteria ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setMinNbSpheresPerBrick( unsigned int pValue )
{
	...
	// Update DEVICE memory with "voxel scale"
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cMinNbSpheresPerBrick, &_minNbSpheresPerBrick, sizeof( _minNbSpheresPerBrick ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setScreenBasedCriteria( bool pFlag )
{
	...
	// Update DEVICE memory with "voxel scale"
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cScreenBasedCriteria, &_screenBasedCriteria, sizeof( _screenBasedCriteria ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setAbsoluteSizeCriteria( bool pFlag )
{
	...
	// Update DEVICE memory with "voxel scale"
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cAbsoluteSizeCriteria, &_absoluteSizeCriteria, sizeof( _absoluteSizeCriteria ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setFixedSizeSphere( bool pFlag )
{
    ...
    // Update DEVICE memory with "voxel scale"
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cFixedSizeSphere, &_fixedSizeSphere, sizeof( _fixedSizeSphere ), 0, cudaMemcpyHostToDevice ) );
...
{
    return /*_pipeline->getProducer()*/_producer->getFixedSizeSphereRadius();
}
...
void SampleCore::setSphereDiameterCoeff( unsigned int pValue )
{
    ...
    // Update DEVICE memory with "voxel scale"
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cCoeffAbsoluteSizeCriteria, &_sphereDiameterCoeff, sizeof( _sphereDiameterCoeff ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setScreenSpaceCoeff( unsigned int pValue )
{
    ...
    // Update DEVICE memory with "voxel scale"
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cScreenSpaceCoeff, &_screenSpaceCoeff, sizeof( _screenSpaceCoeff ), 0, cudaMemcpyHostToDevice ) );
...
{
	/*_pipeline->editProducer()*/_producer->setFixedSizeSphereRadius( pValue );
...
void SampleCore::setMeanSizeOfSpheres( bool pFlag )
{
	...
	// Update DEVICE memory with "voxel scale"
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cMeanSizeOfSpheres, &_meanSizeOfSpheres, sizeof( _meanSizeOfSpheres ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setShaderUniformColorMode( bool pFlag )
{
	...
	// Update DEVICE memory with "voxel scale"
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cShaderUseUniformColor, &_shaderUseUniformColor, sizeof( _shaderUseUniformColor ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setShaderUniformColor( float pR, float pG, float pB, float pA )
{
	...
	// Update DEVICE memory with "voxel scale"
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cShaderUniformColor, &_shaderUniformColor, sizeof( _shaderUniformColor ), 0, cudaMemcpyHostToDevice ) );
}
...
void SampleCore::setShaderAnimation( bool pFlag )
{
	...
	// Update DEVICE memory with "voxel scale"
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cShaderAnimation, &_shaderAnimation, sizeof( _shaderAnimation ), 0, cudaMemcpyHostToDevice ) );
}
...
void SampleCore::setShaderBlurSphere( bool pFlag )
{
    ...
    // Update DEVICE memory with "voxel scale"
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cShaderBlurSphere, &_shaderBlurSphere, sizeof( _shaderBlurSphere ), 0, cudaMemcpyHostToDevice ) );
}
...
void SampleCore::setShaderFog( bool pFlag )
{
    ...
    // Update DEVICE memory with "voxel scale"
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cShaderFog, &_shaderFog, sizeof( _shaderFog ), 0, cudaMemcpyHostToDevice ) );
}
...
void SampleCore::setFogDensity( float pValue )
{
    ...
    // Update DEVICE memory with "voxel scale"
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cShaderFogDensity, &_shaderFogDensity, sizeof( _shaderFogDensity ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setShaderFogColor( float pR, float pG, float pB, float pA )
{
    ...
    ///*_pipeline->editRenderer()*/_renderersetClearColor(make_uchar4(pR*255, pG*255, pB*255, pA*255));

    // Update DEVICE memory with "voxel scale"
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cShaderFogColor, &_shaderFogColor, sizeof( _shaderFogColor ), 0, cudaMemcpyHostToDevice ) );
}
...
void SampleCore::setLightSourceType( bool pFlag )
{
    ...
    // Update DEVICE memory with "voxel scale"
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cShaderLightSourceType, &_shaderLightSourceType, sizeof( _shaderLightSourceType ), 0, cudaMemcpyHostToDevice ) );
...
void SampleCore::setShading( bool pFlag )
{
    ...
    // Update DEVICE memory with "voxel scale"
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cShading, &_shading, sizeof( _shading ), 0, cudaMemcpyHostToDevice ) );
}
...
void SampleCore::setBugCorrection( bool pFlag )
{
    ...
    // Update DEVICE memory with "voxel scale"
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cShaderBugCorrection, &_bugCorrection, sizeof( _bugCorrection ), 0, cudaMemcpyHostToDevice ) );
}
...
{
    return _illuminationCoeff;///*_pipeline->getProducer()*/_producer->getSphereRadiusFader();
}
...
void SampleCore::setIlluminationCoeff( float pValue )
{
    ...
    // Update DEVICE memory with "voxel scale"
    GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cSphereIlluminationCoeff, &_illuminationCoeff, sizeof( _illuminationCoeff ), 0, cudaMemcpyHostToDevice ) );
}
