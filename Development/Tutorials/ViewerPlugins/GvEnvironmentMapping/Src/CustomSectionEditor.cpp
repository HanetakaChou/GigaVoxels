...
:	GvvSectionEditor( pParent, pFlags )
{
	...
	// Editor name
	setName( tr( "Environment Mapping" ) );
}
...
void CustomSectionEditor::populate( GvViewerCore::GvvBrowsable* pBrowsable )
{
	...
	//	_noiseStrengthSpinBox->setValue( pipeline->getNoiseStrength() );
		...
		// Environment mapping
		_reflectionCoeffDoubleSpinBox->setValue( pipeline->getReflectionCoeff() );
		_refractionCoeffDoubleSpinBox->setValue( pipeline->getRefractionCoeff() );
	}
}
...
/******************************************************************************
 * Slot called when the environment mapping's reflection coefficient value has changed
 ******************************************************************************/
void CustomSectionEditor::on__reflectionCoeffDoubleSpinBox_valueChanged( double pValue )
{
	GvvApplication& application = GvvApplication::get();
	GvvMainWindow* mainWindow = application.getMainWindow();
	Gvv3DWindow* window3D = mainWindow->get3DWindow();
	...
	sampleCore->setReflectionCoeff( pValue );
}

/******************************************************************************
 * Slot called when the environment mapping's refraction coefficient value has changed
 ******************************************************************************/
void CustomSectionEditor::on__refractionCoeffDoubleSpinBox_valueChanged( double pValue )
{
	GvvApplication& application = GvvApplication::get();
	GvvMainWindow* mainWindow = application.getMainWindow();
	Gvv3DWindow* window3D = mainWindow->get3DWindow();
	...
	sampleCore->setRefractionCoeff( pValue );
}
