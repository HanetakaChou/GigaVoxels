...
void main()
{
	...
	// Voxel coordinates (i.e. 3D index) where to write
	ivec3 voxCoord = ivec3( 0 );
	if ( uAxe == 0 )
	{
		signe = - sign( dataIn.normal.x * dataIn.distMin );
		voxCoord = ivec3( uSlice, gl_FragCoord.x, gl_FragCoord.y );
	}
	if ( uAxe == 1 )
	{
		signe = - sign( dataIn.normal.y * dataIn.distMin );
		voxCoord = ivec3( gl_FragCoord.y, uSlice, gl_FragCoord.x );
	}
	if ( uAxe == 2 )
	{
		signe = - sign( dataIn.normal.z * dataIn.distMin );
		voxCoord = ivec3( gl_FragCoord.x, gl_FragCoord.y, uSlice );
	}

	// Write min distance
	atomicPutDist(	uDistance[ uAxe ]		/*image (data pool where to write)*/,
					voxCoord				/*voxel coordinates (3D index) where to write*/,
