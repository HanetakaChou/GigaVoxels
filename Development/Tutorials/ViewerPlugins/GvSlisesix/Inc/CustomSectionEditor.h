/*
 * GigaVoxels - GigaSpace
 *
 * Website: http://gigavoxels.inrialpes.fr/
 *
 * Contributors: GigaVoxels Team
 *
 * Copyright (C) 2007-2015 INRIA - LJK (CNRS - Grenoble University), All rights reserved.
 */

/** 
 * @version 1.0
 */

#ifndef CUSTOMSECTIONEDITOR_H
#define CUSTOMSECTIONEDITOR_H

/******************************************************************************
 ******************************* INCLUDE SECTION ******************************
 ******************************************************************************/

// GvViewer
#include <GvvSectionEditor.h>

// Project
#include "UI_GvQCustomEditor.h"

/******************************************************************************
 ************************* DEFINE AND CONSTANT SECTION ************************
 ******************************************************************************/

/******************************************************************************
 ***************************** TYPE DEFINITION ********************************
 ******************************************************************************/

/******************************************************************************
 ******************************** CLASS USED **********************************
 ******************************************************************************/

// GvViewer
namespace GvViewerCore
{
	class GvvBrowsable;
}

/******************************************************************************
 ****************************** CLASS DEFINITION ******************************
 ******************************************************************************/

/** 
 * @class GvvCacheEditor
 *
 * @brief The GvvCacheEditor class provides ...
 *
 * ...
 */
class CustomSectionEditor : public GvViewerGui::GvvSectionEditor, public Ui::GvQCustomEditor
{
	// Qt Macro
	Q_OBJECT

	/**************************************************************************
	 ***************************** PUBLIC SECTION *****************************
	 **************************************************************************/

public:

	/******************************* ATTRIBUTES *******************************/
	
	/******************************** METHODS *********************************/

	/**
	 * Default constructor
	 */
	CustomSectionEditor( QWidget *parent = 0, Qt::WindowFlags flags = 0 );

	/**
	 * Destructor
	 */
	virtual ~CustomSectionEditor();

	/**************************************************************************
	 **************************** PROTECTED SECTION ***************************
	 **************************************************************************/

protected:
	
	/******************************* ATTRIBUTES *******************************/

	/******************************** METHODS *********************************/
	
	/**
	 * Populates this editor with the specified browsable
	 *
	 * @param pBrowsable specifies the browsable to be edited
	 */
	virtual void populate( GvViewerCore::GvvBrowsable* pBrowsable );
	
	/**************************************************************************
	 ***************************** PRIVATE SECTION ****************************
	 **************************************************************************/

private:
	
	/******************************* ATTRIBUTES *******************************/
	
	/******************************** METHODS *********************************/

private slots:

	/**
	 * Slot called when color tool button is released
	 */
	void on__colorToolButton_released();

	/**
	 * Slot called when opacity value has changed
	 */
	void on__opacitySpinBox_valueChanged( double pValue );

	/**
	 * Slot called when shader material property (according to opacity) value has changed
	 */
	void on__shaderMaterialPropertySpinBox_valueChanged( double pValue );

	/**
	 * Slot called when ceiling distance field state has changed
	 */
	void on__ceilingCheckBox_stateChanged( int pValue );

	/**
	 * Slot called when floor distance field state has changed
	 */
	void on__floorCheckBox_stateChanged( int pValue );

	/**
	 * Slot called when columns distance field state has changed
	 */
	void on__columnsCheckBox_stateChanged( int pValue );

	/**
	 * Slot called when monster distance field state has changed
	 */
	void on__monsterCheckBox_stateChanged( int pValue );

	/**
	 * Slot called when eye position has changed
	 */
	void on__eyeXDoubleSpinBox_valueChanged( double value );

	/**
	 * Slot called when eye position has changed
	 */
	void on__eyeYDoubleSpinBox_valueChanged( double value );

	/**
	 * Slot called when eye position has changed
	 */
	void on__eyeZDoubleSpinBox_valueChanged( double value );

	/**
	 * Slot called when map scale has changed
	 */
	void on__mapScaleDoubleSpinBox_valueChanged( double value );

	/**
	 * Slot called when bump mapping state has changed
	 */
	void on__bumpMappingCheckBox_stateChanged( int pValue );

	/**
	 * Slot called when ambient occlusion state has changed
	 */
	void on__ambientOcclusionCheckBox_stateChanged( int pValue );

	/**
	 * Slot called when soft shadows state has changed
	 */
	void on__softShadowsCheckBox_stateChanged( int pValue );

};

#endif
