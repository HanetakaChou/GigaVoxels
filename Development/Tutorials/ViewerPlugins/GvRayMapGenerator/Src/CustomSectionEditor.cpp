...
void CustomSectionEditor::populate( GvViewerCore::GvvBrowsable* pBrowsable )
{
	...
	if ( pipeline != NULL )
	{
		...
		// Ray map parameters
		_rayMapTypeComboBox->setCurrentIndex( pipeline->getRayMapType() );
	}
}
...
/******************************************************************************
 * Slot called when ray map generator's type value has changed
 ******************************************************************************/
void CustomSectionEditor::on__rayMapTypeComboBox_currentIndexChanged( int pIndex )
{
	GvvApplication& application = GvvApplication::get();
	GvvMainWindow* mainWindow = application.getMainWindow();
	Gvv3DWindow* window3D = mainWindow->get3DWindow();
	...
	// Update GigaVoxels pipeline
	sampleCore->setRayMapType( pIndex );
}
