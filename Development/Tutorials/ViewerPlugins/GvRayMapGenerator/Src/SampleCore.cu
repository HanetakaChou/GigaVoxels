...
void SampleCore::init()
{
	...
	QString dataRepository = QCoreApplication::applicationDirPath() + QDir::separator() + QString( "Data" ) + QDir::separator() + QString( "Shaders" ) + QDir::separator() + QString( "GvRayMapGenerator" ) + QDir::separator();
	QString vertexShaderFilename = dataRepository + QString( "rayMap_vert.glsl" );
	QString fragmentShaderFilename = dataRepository + QString( "rayMap_frag.glsl" );
...
void SampleCore::resetGraphicsresources()
{
	...
	_rayMap->initialize();
	_rayMap->setResolution( _graphicsEnvironment->getBufferWidth(), _graphicsEnvironment->getBufferHeight() );
...
{
	return _rayMap->getShaderProgram()->hasShaderType( static_cast< GvShaderProgram::ShaderType >( pShaderType ) );
}
...
{
	return _rayMap->getShaderProgram()->getShaderSourceCode( static_cast< GvShaderProgram::ShaderType >( pShaderType ) );
}
...
{
	return _rayMap->getShaderProgram()->getShaderFilename( static_cast< GvShaderProgram::ShaderType >( pShaderType ) );
}
...
{
	return _rayMap->editShaderProgram()->reloadShader( static_cast< GvShaderProgram::ShaderType >( pShaderType ) );
}
...
void SampleCore::setRayMapType( unsigned int pValue )
{
	...
	// ...
	_rayMap->setRayMapType( static_cast< GvRayMap::RayMapType >( pValue ) );
}
