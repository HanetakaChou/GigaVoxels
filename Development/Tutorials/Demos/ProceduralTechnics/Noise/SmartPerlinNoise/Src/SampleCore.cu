 ...
 ******************************************************************************/
void SampleCore::setNoiseParameters( float p1, float p2 )
{
	// Update DEVICE memory with "noise parameters"
	//
	float2 noiseParams = make_float2( p1, p2 );
	GS_CUDA_SAFE_CALL( cudaMemcpyToSymbol( cNoiseParameters, &noiseParams, sizeof( noiseParams ), 0, cudaMemcpyHostToDevice ) );
}
