...
,	_transferFunctionEditor( NULL )
{
	...
	// Window title
	setWindowTitle( tr( "Smart Noise on device (noise upper bound) : amplified Surface example" ) );
}
...
void SampleViewer::keyPressEvent( QKeyEvent* e )
{
	...
	case Qt::Key_5:
				// Used to increase the first noise amplitude
			_noiseParameters[ 1 ] -= 0.001f;
			if ( _noiseParameters[ 1 ] < 0.f )
				_noiseParameters[ 1 ] = 0.f;
