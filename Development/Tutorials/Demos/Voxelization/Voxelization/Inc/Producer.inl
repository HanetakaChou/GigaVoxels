...
::initialize( GvStructure::GsIDataStructure* pDataStructure, GvStructure::GsIDataProductionManager* pDataProductionManager )
{
	...
	_requestListCode = new GvCore::GsLocalizationInfo::CodeType[ nbMaxRequests ];
	_requestListDepth = new GvCore::GsLocalizationInfo::DepthType[ nbMaxRequests ];

	_requestListAddress = new uint[ nbMaxRequests ];

	// The following two buffers are their equivalents on GPU
	_requestListCodeDevice = new thrust::device_vector< GvCore::GsLocalizationInfo::CodeType >( nbMaxRequests );
	_requestListDepthDevice = new thrust::device_vector< GvCore::GsLocalizationInfo::DepthType >( nbMaxRequests );
	...
	QString meshFilename = GvUtils::GsEnvironment::getDataDir( GvUtils::GsEnvironment::e3DModelsDir ).c_str();
	meshFilename += QDir::separator();
	meshFilename += QString( "bunny.obj" );
	_scene.init( meshFilename.toLatin1().constData() ); //TO DO : have to be a parameter of the producer
	...
	//_voxelizationProgram = GvUtils::GsShaderManager::createShaderProgram( "Data/Shaders/Voxelization/voxelization_VS.glsl", "Data/Shaders/Voxelization/voxelization_GS.glsl", "Data/Shaders/Voxelization/voxelization_FS.glsl" );
	QString shaderRepository = GvUtils::GsEnvironment::getDataDir( GvUtils::GsEnvironment::eShadersDir ).c_str();
	shaderRepository += QDir::separator();
	shaderRepository += QString( "Voxelization" );
	QString vertexShaderFilename = shaderRepository;
	vertexShaderFilename += QDir::separator();
	vertexShaderFilename += QString( "voxelization_VS.glsl" );
	QString geometryShaderFilename = shaderRepository;
	geometryShaderFilename += QDir::separator();
	geometryShaderFilename += QString( "voxelization_GS.glsl" );
	QString fragmentShaderFilename = shaderRepository;
	fragmentShaderFilename += QDir::separator();
	fragmentShaderFilename += QString( "voxelization_FS.glsl" );
	_voxelizationProgram = GvUtils::GsShaderManager::createShaderProgram( vertexShaderFilename.toLatin1().constData(), geometryShaderFilename.toLatin1().constData(), fragmentShaderFilename.toLatin1().constData() );
	GvUtils::GsShaderManager::linkShaderProgram( _voxelizationProgram );
				...
				Loki::Int2Type< 0 > )
{
	...
	// Retrieve raw pointers from device_vectors
	GvCore::GsLocalizationInfo::CodeType* locCodeList = thrust::raw_pointer_cast( &(*_requestListCodeDevice)[0] );
	GvCore::GsLocalizationInfo::DepthType* locDepthList = thrust::raw_pointer_cast( &(*_requestListDepthDevice)[0] );
...
::produceNodes( const uint pNbElements, const GvCore::GsLocalizationInfo::CodeType* pRequestListCodePtr, const GvCore::GsLocalizationInfo::DepthType* pRequestListDepthPtr )
{
	...
	cudaMemcpy( _requestListCode, pRequestListCodePtr, pNbElements * sizeof( GvCore::GsLocalizationInfo::CodeType ), cudaMemcpyDeviceToHost );
	cudaMemcpy( _requestListDepth, pRequestListDepthPtr, pNbElements * sizeof( GvCore::GsLocalizationInfo::DepthType ), cudaMemcpyDeviceToHost );
...
template< typename TDataStructureType, typename TDataProductionManager >
inline void Producer< TDataStructureType, TDataProductionManager >
::produceBricks( const uint numElements, const GvCore::GsLocalizationInfo::CodeType* pRequestListCodePtr, const GvCore::GsLocalizationInfo::DepthType* pRequestListDepthPtr, const uint* pElemAddressListPtr )
{
	// TODO : use asynchronous data transfert

	// Retrieve localization info lists from device to host
	cudaMemcpy( _requestListCode, pRequestListCodePtr, numElements * sizeof( GvCore::GsLocalizationInfo::CodeType ), cudaMemcpyDeviceToHost );
	cudaMemcpy( _requestListDepth, pRequestListDepthPtr, numElements * sizeof( GvCore::GsLocalizationInfo::DepthType ), cudaMemcpyDeviceToHost );
