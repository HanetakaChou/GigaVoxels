...
void main()
{
	...
	for ( int slice = 0; slice < 10; slice++ )
	{
		...
		texelDist.x = imageLoad( uDistanceX, ivec3( gl_FragCoord.x, gl_FragCoord.y, slice ) ).r;
		texelDist.y = imageLoad( uDistanceY, ivec3( gl_FragCoord.x, gl_FragCoord.y, slice ) ).r;
		texelDist.z = imageLoad( uDistanceZ, ivec3( gl_FragCoord.x, gl_FragCoord.y, slice ) ).r;
