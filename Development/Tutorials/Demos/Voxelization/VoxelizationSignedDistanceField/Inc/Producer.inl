...
::initialize( GvStructure::GsIDataStructure* pDataStructure, GvStructure::GsIDataProductionManager* pDataProductionManager )
{
	...
	requestListCode = new GvCore::GsLocalizationInfo::CodeType[ nbMaxRequests ];
	requestListDepth = new GvCore::GsLocalizationInfo::DepthType[ nbMaxRequests ];
	requestListAddress = new uint[ nbMaxRequests ];

	// The following two buffers are their equivalents on GPU
	requestListCodeDevice = new thrust::device_vector< GvCore::GsLocalizationInfo::CodeType >( nbMaxRequests );
	requestListDepthDevice = new thrust::device_vector< GvCore::GsLocalizationInfo::DepthType >( nbMaxRequests );
	...
	mScene.init( meshFilename.toLatin1().constData() );
	...
	shaderRepository += QString( "VoxelizationSignedDistanceField" );

	// Initialize shader program
	//mDistProg = GvUtils::GsShaderManager::createShaderProgram( "Data/Shaders/VoxelizationSignedDistanceField/signedDist_VS.glsl", NULL, "Data/Shaders/VoxelizationSignedDistanceField/signedDist_FS.glsl" );
	QString vertexShaderFilename = shaderRepository;
	vertexShaderFilename += QDir::separator();
	vertexShaderFilename += QString( "signedDist_VS.glsl" );
	QString fragmentShaderFilename = shaderRepository;
	fragmentShaderFilename += QDir::separator();
	fragmentShaderFilename += QString( "signedDist_FS.glsl" );
	mDistProg = GvUtils::GsShaderManager::createShaderProgram( vertexShaderFilename.toLatin1().constData(), NULL, fragmentShaderFilename.toLatin1().constData() );
	GvUtils::GsShaderManager::linkShaderProgram( mDistProg );

	// Initialize shader program
	//mPotentialProg = GvUtils::GsShaderManager::createShaderProgram( "Data/Shaders/VoxelizationSignedDistanceField/potentialFunc_VS.glsl", NULL, "Data/Shaders/VoxelizationSignedDistanceField/potentialFunc_FS.glsl" );
	vertexShaderFilename = shaderRepository;
	vertexShaderFilename += QDir::separator();
	vertexShaderFilename += QString( "potentialFunc_VS.glsl" );
	fragmentShaderFilename = shaderRepository;
	fragmentShaderFilename += QDir::separator();
	fragmentShaderFilename += QString( "potentialFunc_FS.glsl" );
	mPotentialProg = GvUtils::GsShaderManager::createShaderProgram( vertexShaderFilename.toLatin1().constData(), NULL, fragmentShaderFilename.toLatin1().constData() );
	GvUtils::GsShaderManager::linkShaderProgram( mPotentialProg );
...
::produceSignedDistanceField( float3 pBrickPos, float pXSize, float pYSize, float pZSize, unsigned int pDepth, uint3 pLocCode, uint3 pAddressBrick )
{
	...
	// OpenGL 4.5 new feature (DSA : directe state access)
	const float tmpData = 1.0f;
	for ( int i = 0; i < 3; ++i )
	{
		glClearTexImage( _distanceTexture[ i ], 0, GL_RED, GL_FLOAT, &tmpData );
	}
	GV_CHECK_GL_ERROR();
	...
	{	    	
		...
		// On Fermi (GTX 470), it seems to work without barriers,
		// but not on Kepler (GTX 670).
		// TODO : check type of barrier(s) to optimize code
